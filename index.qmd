...
*What patterns do you observe in the Mann-Kendall results across nearby stations? Are these results climatologically plausible? What does this suggest about single-station trend analysis?* The results vary across stations. Stations that are almost on top of eachother are very similar for S Statistics and p-values. But there is high variation when increasing the distance. This cannot be explained by changing climatology, but is likely due to randomness of which specific areas where hit with thunderstorms. A single-station trend analysis may be heavily dominated by the randomness of the data instead of picking up on actual regiona changes in climate.

**Response 2: Nonstationary Model Uncertainty**
*How does posterior uncertainty in return levels compare across the two nonstationary GEV models? Which model structure provides the most reliable estimates and why?* The two models are very similar. This suggests that allowing the scale parameter to vary with CO2 does not significantly change the projected return level. There is more variation in the 100-year RL for the Location + Scale model. Therefore, it may be preferable to use the Location only model to create more reliable estimates. It makes intuitive sense that varying only one parameter would generate more consistent estimates. 
...
When analyzing a single-station, the model can fit very closely to the data resulting in low-bias. However, it will also fit the noise resulting in high variance. Conversely, regional analysis will limit the impact of the noise, low variance, but may be biased by other stations. This is especially true if the stations are not climatologically similar for some reason, for example a mountain range. The single-station method is probably best where the station has a long history of data and the other regional stations are not likely to have similar climatology or have many errors (ie faulty during hurricanes). The regional analysis is better if there are multiple stations with similar climatology but short-datasets. It can also capture regional changes better. 
...
The return levels remain about the same using single-station v.regional parameter estimates. The uncertainty n the predictions is reduced throughout the return period. This allows us to be more confident estimating return levels when there is limited data at a particular station. 

**Response 5: Regional Analysis for PS1**

I will use th s analysis to complete Task 4. The techniques will help me analyze the return levels of rainfall at the Galveston sation and leverage other stations to reduce the noise and uncertainty in the estimates. I anticipate challenges in ensuring that the stations selected reflect the climatology of the Galveston region. Further inland may have different weather patterns than strictly along the coast. 
...
```{julia}
my_stnid = 779  # Station 41-4309 (Houston Addicks), selected for potential trend signal
